...
export const renderTable = (data: TOpenOrderData[], prices: TSymbolPrice[]) => {
    ...
    for (const order of coll) {
        const currentPrice = getPriceByPairName(prices, order.symbol)
        ...
        table.cell(chalk.green('Current Price'), currentPrice)
        table.cell(chalk.green('Difference'), formatIndicativePercentage((currentPrice - order.price) / currentPrice))
